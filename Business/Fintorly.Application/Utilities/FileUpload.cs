...
public static class FileUpload
{
    ...
    public static IResult Upload(IFormFile file, string folderName)
    {
        ...
        return Result.Success((_currentDirectory + $"\\{folderName}\\" + randomName + type).Replace("\\", "/"),
            randomName + type + file);
    }

    public static IResult<UploadResult> UploadAlternative(IFormFile file, string folderName, string? oldFilePath = "")
    {
        ...
        var typeValid = CheckFileTypeValid(type);
        if (!typeValid.Succeeded)
            return Result<UploadResult>.Fail(typeValid.Message);
        ...
        return Result<UploadResult>.Success(uploadResult);
    }
